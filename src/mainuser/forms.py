...
class DatosPersonaForm(forms.ModelForm):
    ...
    IdEtnias = forms.ModelChoiceField(label="Etnia",queryset=Etnia.objects.all(),empty_label="Seleccione su etnia",required=True,widget=forms.Select(attrs={'class':'form-control'}),)
    IdEstaCivil = forms.ModelChoiceField(label="Estado civil",queryset=EstaCivil.objects.all(),empty_label="Seleccione su estado civíl",required=True,widget=forms.Select(attrs={'class':'form-control'}),)
